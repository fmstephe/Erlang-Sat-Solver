...
assign_variable(Variable_Assignment,Formula) ->
	...
	{New_Formula,Unit_Literals} = check_clauses(Inc_Formula,[Variable_Assignment]),
...
assign_variables(Variable_Assignments,Formula) ->
    Inc_Formula = lists:foldl(fun set_variable/2,Formula,Variable_Assignments),
	{New_Formula,Unit_Literals} = check_clauses(Inc_Formula,Variable_Assignments),
    case Unit_Literals of
        []
        	->	New_Formula;
        Unit_Literals
...
%%
check_clauses(Formula,Variable_Assignments) ->
    Clauses = Formula#formula.clauses,
    Variable_Dict = Formula#formula.variable_dict,
    {New_Clauses,Unit_Literals} = check_clauses(Clauses,Variable_Assignments,[],[]),
	{#formula{variable_dict=Variable_Dict,clauses=New_Clauses},Unit_Literals}.

%%
%%	
%%
check_clauses([],_Variable_Assignments,New_Clauses,Unit_Literals) ->
    {New_Clauses,Unit_Literals};
    
check_clauses([Clause|Clauses],Variable_Assignments,New_Clauses,Unit_Literals) ->
    {New_Clause,Unit_Literal} = check_clause(Clause,Variable_Assignments),
    case {New_Clause,Unit_Literal} of
        {subsumed,?NULL_LITERAL}
        	->	check_clauses(Clauses,Variable_Assignments,New_Clauses,Unit_Literals);
        {New_Clause,?NULL_LITERAL}
        	->	check_clauses(Clauses,Variable_Assignments,[New_Clause|New_Clauses],Unit_Literals);
        {New_Clause,Unit_Literal}
        	->	check_clauses(Clauses,Variable_Assignments,[New_Clause|New_Clauses],[Unit_Literal|Unit_Literals])
    end.
...
%%		#clause
%%
check_clause(Clause,Variable_Assignments) ->
    Literals = Clause#clause.literals,
    case are_literals_indetermined(Literals,Variable_Assignments) of
        true
        	->	{Clause,?NULL_LITERAL};
        false
          	->	modify_clause(Clause,Variable_Assignments)
    end.
...
%%		[#Literal]
%%
are_literals_indetermined([],_Variable_Assignments) -> true;

are_literals_indetermined([Literal|Literals],Variable_Assignments) ->
    case get_set_value(Literal,Variable_Assignments) of
        indetermined
        	->	are_literals_indetermined(Literals,Variable_Assignments);
    	_True_Or_False
			->	false
    end.
...
%%	
%%
get_set_value(_Literal,[]) ->
    indetermined;

get_set_value(Literal,[Variable_Assignment|_Variable_Assignments])
  when Literal#literal.l_name =:= Variable_Assignment#variable_assignment.l_name ->
    Variable_Assignment#variable_assignment.value;

%%	As a trick it is possible for Variable Assignments to be passed in as literals
%%	I should probably get rid of this.
get_set_value(Literal,[Variable_Assignment|_Variable_Assignments])
  when Literal#literal.l_name =:= Variable_Assignment#literal.l_name ->
    Variable_Assignment#literal.negation;

get_set_value(Literal,[_Variable_Assignment|Variable_Assignments]) ->
    get_set_value(Literal,Variable_Assignments).

%%
%%	
%%
modify_clause(Clause,Variable_Assignments) ->
	Literals = check_literals(Clause#clause.literals,Variable_Assignments),
	case Literals of
        subsumed
        	->	{subsumed,?NULL_LITERAL};
        [Literal|[]]
...
check_literals([],_Variable_Assignments) -> [];

check_literals([Literal|Literals],Variable_Assignments) ->
    Value = get_set_value(Literal,Variable_Assignments),
    Negation = Literal#literal.negation,
	case ternary_logic:not3(ternary_logic:xor3(Negation,Value)) of
			true 
				-> subsumed;
			false
				-> check_literals(Literals,Variable_Assignments);
			indetermined
				-> Other_Literals = check_literals(Literals,Variable_Assignments),
